Internal Network — Private Organizational Messaging Platform
============================================================

Welcome to **MPSTME ACM Internal Network**, a private, real-time collaboration platform inspired by Discord and Slack, purpose-built for engineering clubs and internal technical communities.

This project is optimized for **MPSTME ACM** with a dark, developer-tool aesthetic and role-based controls for admins, moderators, and members.

Project Description
-------------------

**Internal Network** is a real-time messaging platform where users collaborate in rooms such as `#acm-general`, `#acm-backend`, `#acm-frontend`, and `#acm-announcements`. The system is designed for **private organizational use**, with strict role-based permissions and a tech-conference inspired UI.
...
- **Admin-style controls** via roles and commands
...
- **Database**: MongoDB (MongoDB Atlas recommended), Mongoose
...
- `config/`
  - `default.js` – configuration (JWT secret, expiry)
- `controllers/`
  - `authController.js` – register/login
  - `roomController.js` – room CRUD actions
  - `messageController.js` – message history
- `models/`
  - `User.js` – user schema with roles and banned flag
  - `Room.js` – room schema with members and creator
...
- `middleware/`
  - `authMiddleware.js` – JWT validation, role guard, Socket.io auth
- `sockets/`
  - `chatSocket.js` – socket event handlers (join, leave, send, typing, reaction)
- `commands/`
  - `kick.js` – `/kick username`
  - `ban.js` – `/ban username`
  - `clear.js` – `/clear`
  - `createRoom.js` – `/create-room roomName`
- `utils/`
  - `jwt.js` – token helpers
  - `commandParser.js` – parse and dispatch `/commands`
...
- `components/`
  - `ChatWindow.tsx` – main chat panel for a room
  - `Message.tsx` – individual message with role badge and reactions
  - `MessageInput.tsx` – input box with command hint and typing indicator
  - `RoomList.tsx` – sidebar list of rooms
  - `UserList.tsx` – online users panel
  - `TypingIndicator.tsx` – “user is typing” display
  - `admin/RoomManager.tsx` – admin room CRUD UI
  - `admin/UserManager.tsx` – admin user/role/ban management UI
  - `admin/ActivityMonitor.tsx` – recent messages and command monitor
- `context/`
  - `AuthContext.tsx` – auth state, login/register/logout, JWT
  - `SocketContext.tsx` – Socket.io connection bound to auth token
- `services/`
  - `api.ts` – Axios API functions for auth, rooms, messages, users, activity
- `pages/`
  - `_app.tsx` – app wrapper with providers
  - `index.tsx` – redirects to `Login` / `Dashboard` based on auth
  - `Login.tsx` – login/register page
  - `Dashboard.tsx` – main chat dashboard
  - `AdminPanel.tsx` – admin-only control center
- `styles/`
  - `globals.css` – Tailwind base + ACM-inspired theme
...
- `joinedRooms`: ObjectId[] (references `Room`)
- `banned`: boolean
...
Namespace: default Namespace on backend Socket.io server.
...
- `receiveMessage` – `{ _id, roomId, sender, content, reactions, timestamp }`
- `typing` – `{ roomId, user, isTyping }`
- `reaction` – `{ messageId, emoji, userId }`
- `systemMessage` – `{ roomId?, content }`
...
- **Tech conference style**: gradient glows, ACM-blue and teal accents
...
2. **Seed the database (optional but recommended)**:
   ...
   - Default rooms: `#acm-general`, `#acm-backend`, `#acm-frontend`, `#acm-announcements`
...
- **Admin**
  - Create rooms
  - Delete rooms
  - Assign roles (future extension)
  - Remove users (via `/kick`, `/ban`)
  - Access admin-like commands
- **Moderator**
  - Create rooms
  - Delete messages (`/clear`)
  - Remove users from rooms (`/kick`)
  - Assign limited roles (future extension)
- **User**
  - Join rooms
  - Send messages
...
- Admins can access a dedicated **Admin Panel** at `/AdminPanel` (there is also a button in the main dashboard header for admins).
...
> If this repository is **private**, please add **`@ACM-MPSTME`** as a collaborator so judges can access the codebase.

Screenshots (Placeholders)
--------------------------

Add screenshots/gifs to showcase:

- Login / Welcome Screen (dark tech-conference aesthetic)
- Main Dashboard with rooms sidebar, chat window, and user list
- Commands in action (`/create-room`, `/kick`, `/ban`, `/clear`)
- Typing indicators and message reactions
...
- `screenshots/reactions-typing.png`
